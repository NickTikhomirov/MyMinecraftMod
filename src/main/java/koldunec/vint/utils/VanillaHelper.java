...
public class VanillaHelper {
    ...
    public static void FillStORE(EntityStore p1, World worldIn, ResourceLocation l){
        if(worldIn.isRemote)
            return;
        LootContext.Builder builder = new LootContext.Builder((WorldServer) worldIn);
        worldIn.getLootTableManager().getLootTableFromLocation(l).fillInventory(p1, vint.random, builder.build());
    }
}
